...
function renderPair(d){
  ...
  window.CC_SEARCH_ANSWER.write(out,`<div class="result-card cc245-card" data-cc221="1"><div class="label">COMPARE · 척척</div><h3>${esc(d.title)}</h3><div class="cc245-head"><div><small>${esc(d.left)}</small><b>${esc(d.one[0])}</b></div><em>VS</em><div><small>${esc(d.right)}</small><b>${esc(d.one[1])}</b></div></div><div class="cc245-table"><div class="cc245-tr cc245-th"><span>구분</span><span>${esc(d.left)}</span><span>${esc(d.right)}</span></div>${d.rows.map(r=>`<div class="cc245-tr"><b>${esc(r[0])}</b><span>${esc(r[1])}</span><span>${esc(r[2])}</span></div>`).join('')}</div><div class="cc245-first"><small>먼저 확인</small><b>${esc(d.first)}</b></div><div class="cc245-more"><div class="cc245-caution"><b>주의</b><span>${esc(d.caution)}</span></div><div class="cc245-sources">${links(d.sources)}</div></div><div class="cc245-suggest"><small>다른 비교</small><button data-cc245-query="변경신고와 경미한 변경의 차이가 뭐야?">변경신고 vs 경미한 변경</button><button data-cc245-query="건축허가와 건축신고의 차이가 뭐야?">건축허가 vs 건축신고</button></div></div>`,window.CC_SEARCH_ANSWER.comparison(d.title,[d.left,d.one[0]],[d.right,d.one[1]],d.first,d.caution));
  wire(out);return true;
}
function renderUnknown(){
  const out=$('searchResult');if(!out)return false;
  window.CC_SEARCH_ANSWER.write(out,`<div class="result-card cc245-card cc245-unknown" data-cc221="1"><div class="label">COMPARE · 척척</div><h3>비교 질문은 인식했지만, 아직 검증된 비교 조합이 아니에요</h3><p>관련 없는 일반 답변을 만들지 않고 비교할 두 용어를 다시 좁힐게요.</p><div class="cc245-known"><button data-cc245-query="변경허가와 변경신고의 차이가 뭐야?">변경허가 vs 변경신고</button><button data-cc245-query="변경신고와 경미한 변경의 차이가 뭐야?">변경신고 vs 경미한 변경</button><button data-cc245-query="건축허가와 건축신고의 차이가 뭐야?">건축허가 vs 건축신고</button><button data-cc245-query="건축심의와 경관심의의 차이가 뭐야?">건축심의 vs 경관심의</button></div><div class="cc245-caution"><b>답변 원칙</b><span>법적 기준이 다른 용어는 정의·판단기준·처리시점·실무 출발점을 확인한 조합부터 제공합니다.</span></div></div>`);
  wire(out);return true;
}
function wire(root){
