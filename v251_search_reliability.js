...
function renderDefinition(route){
  ...
  window.CC_SEARCH_ANSWER.write(out,`<div class="result-card cc253-card" data-cc253="definition"><div class="label">DEFINITION · ${esc(audienceLabel(route.persona))}</div><h3>${esc(item.label)}, 이렇게 이해하면 돼요</h3><p>${esc(item.meaning)}</p><div class="result-grid"><div class="result-cell"><small>실무에서 무엇을 하나요?</small><p>${esc(item.scope)}</p></div><div class="result-cell"><small>무엇부터 확인하나요?</small><p>${esc(item.first)}</p></div><div class="result-cell"><small>누구에게 확인하나요?</small><p>${esc(who)}</p></div></div><div class="cc253-caution"><b>주의</b><span>${esc(item.caution)}</span></div><div class="cc253-sources">${sourceLinks(item.sources)}</div></div>`,window.CC_SEARCH_ANSWER.model(item.label+', 이렇게 이해하면 돼요',[['실무에서 무엇을 하나요?',item.scope],['무엇부터 확인하나요?',item.first],['누구에게 확인하나요?',who]]));
}
...
function renderComparison(route){
  ...
  const out=$('searchResult');if(!out)return;
  window.CC_SEARCH_ANSWER.write(out,`<div class="result-card cc245-card cc253-card" data-cc253="comparison"><div class="label">COMPARE · ${esc(audienceLabel(route.persona))}</div><h3>${esc(title)}</h3><div class="cc245-head"><div><small>${esc(left.label)}</small><b>${esc(left.meaning)}</b></div><em>VS</em><div><small>${esc(right.label)}</small><b>${esc(right.meaning)}</b></div></div><div class="cc245-table"><div class="cc245-tr cc245-th"><span>구분</span><span>${esc(left.label)}</span><span>${esc(right.label)}</span></div><div class="cc245-tr"><b>핵심 범위</b><span>${esc(left.scope)}</span><span>${esc(right.scope)}</span></div><div class="cc245-tr"><b>처리·확인 주체</b><span>${esc(left.authority)}</span><span>${esc(right.authority)}</span></div><div class="cc245-tr"><b>확인 시점</b><span>${esc(left.timing)}</span><span>${esc(right.timing)}</span></div></div><div class="cc245-first"><small>먼저 확인</small><b>${esc(first)}</b></div><div class="cc245-more"><div class="cc245-caution"><b>주의</b><span>${esc(caution)}</span></div><div class="cc245-sources">${sourceLinks(sources)}</div></div></div>`,window.CC_SEARCH_ANSWER.comparison(title,[left.label,left.meaning],[right.label,right.meaning],first,caution));
}
