...
function renderChooser(special=false){
  ...
  window.CC_SEARCH_ANSWER.write(out,`<div class="result-card cc243-chooser" data-cc221="1"><div class="label">CHANGE ROUTE · 척척</div><h3>${special?'어떤 특별법·개발사업의 변경인가요?':'변경업무는 원래 어떤 절차로 승인받았는지부터 골라주세요'}</h3><p>${special?'시설명보다 기존 실시계획·단지계획·승인서의 법적 근거가 기준입니다.':'같은 도면 변경도 건축허가, 주택법, 정비사업, 특별법 사업에 따라 절차가 달라져요.'}</p><div class="cc243-routes">${routeButtons(items)}</div></div>`);
...
function renderGuide(key){
  ...
  window.CC_SEARCH_ANSWER.write(out,`<div class="result-card cc243-card" data-cc221="1" data-cc243-guide="${esc(key)}"><div class="label">CHANGE HOW · LV3 · 척척</div><h3>${esc(d.title)}</h3>${ctx?`<div class="cc243-context">현재 선택 · ${esc(ctx)}</div>`:''}${d.classification?`<div class="cc243-classification">업무 분류 · ${esc(d.classification)}</div>`:''}<div class="cc243-tag">${esc(d.tag)}</div><p class="cc243-summary">${esc(d.summary)}</p><div class="cc243-steps"><small>지금 할 일</small>${d.steps.map(([t,b],i)=>`<div><em>${i+1}</em><section><b>${esc(t)}</b><p>${esc(b)}</p></section></div>`).join('')}</div><div class="cc243-done"><small>변경비교표 최소 항목</small><b>변경항목 · 기존 승인내용 · 변경내용 · 영향도서 · 협의대상 · 절차판단 · 처리시점</b></div><div class="cc243-more"><div class="cc243-more-steps">${d.more.map(([t,b],i)=>`<div><em>${i+4}</em><section><b>${esc(t)}</b><p>${esc(b)}</p></section></div>`).join('')}</div><div class="cc243-caution"><b>주의</b><span>${esc(d.caution)}</span></div><a class="cc243-source" href="${esc(source[1])}" target="_blank" rel="noopener noreferrer">${esc(source[0])} ↗</a></div><div class="cc243-switch"><small>다른 승인경로 보기</small><button data-cc243-back>승인경로 다시 선택</button></div></div>`,window.CC_SEARCH_ANSWER.model(d.title,d.steps));
