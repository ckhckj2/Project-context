...
function install(){window.ccSearchRouterClassify=q=>classify(q).id+(classify(q).kind?':'+classify(q).kind:'');window.addEventListener('click',capture,true);window.addEventListener('keydown',capture,true);style();}
