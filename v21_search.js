...
function enhancedRunSearch(){
 const input=$('searchInput'),out=$('searchResult');if(!input||!out)return;
 const q=input.value.trim();if(!q)return;
 let html='',answer=null;
 const known=knownTerm(q);
 if(ASK.test(q)){html=askAnswer(q);}
 else if(TERM.test(q)&&(known||typeof window.termRoute==='function')){
  const term=known||window.termRoute(q);html=renderTerm(term);
  answer=window.CC_SEARCH_ANSWER.model(term.key,[['뜻',term.desc],['같이 기억',term.next]]);
 }else{
  const hit=resolveIntent(q);
  if(hit?.choices)html=renderChoices(hit);
  else if(hit){html=renderTaskCard(hit);answer=window.CC_SEARCH_ANSWER.work(hit);}
  else html=renderChoices(genericChoices(q));
 }
 window.CC_SEARCH_ANSWER.write(out,html,answer);
...
function install(){if(document.body.dataset.cc21==='1')return;document.body.dataset.cc21='1';  window.CC_RUNTIME.registerSearch('fallback',()=>true,()=>enhancedRunSearch());}
window.CC_BOOT.register('v21_search',install);
})();
