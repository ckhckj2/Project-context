...
function preCapture(e){const q=queryFromEvent(e);if(!q)return;pendingQuery=q.trim();setTimeout(()=>renderAdjustment(pendingQuery),280)}
...
function install(){installStyle();window.addEventListener('click',preCapture,true);window.addEventListener('keydown',preCapture,true);document.querySelectorAll('.version').forEach(v=>v.textContent='v'+VERSION)}
