...
const ROUTES={
  building:{label:'건축허가·신고 변경',query:'변경허가 절차는 어떻게 해?',extraChecks:['변경 전 처리 필요 여부','경미한 변경 또는 사용승인 시 일괄신고 가능성','기존 허가조건·의제협의 재검토']},
...
function install(){
  installStyle();document.querySelectorAll('.version').forEach(v=>v.textContent='v'+VERSION);attach();
